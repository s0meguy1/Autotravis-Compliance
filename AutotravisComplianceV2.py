...
def get_compliance(file):
    ...
    failedItems.to_csv('output2.csv', index=False)
    failedItems = failedItems.drop_duplicates()
    # failedItems = failedItems.drop_duplicates(subset='DescriptionGroup')
    failedItems.to_csv('output3.csv', index=False)
    sorteditems = failedItems.sort_values('DescriptionGroup')
    # for item in failedItems.iterrows():
    #     print("----", item)
        # if(str(item["Plugin ID"]) == "1000157"):
        #    print(item["Plugin ID"], item["Severity"])
    testcount = 1
    ...
    #################################
        ...
        caterogry_string = str(row['Description']).split("CAT #",1)[1].split(",")[0]
        if(row["Plugin ID"] == "1000157"):
           print(row["Plugin ID"], row["Severity"])
        if(caterogry_string == "I"):
            print("------", row["Plugin ID"], caterogry_string)
            worksheet.write('D' + str(testcount), "HIGH", normaltext)
        elif(caterogry_string == "II"):
            worksheet.write('D' + str(testcount), "MEDIUM", normaltext)
        else:
            worksheet.write('D' + str(testcount), "LOW", normaltext)
        ...
        try: 
            #removes duplicate host name and solution fields from description.
            #evenBetterDescription = str(row['Description']).split("FAILED]\n\n",1)[1].split("Solution")[0]
            evenBetterDescription = str(row['Description'])
